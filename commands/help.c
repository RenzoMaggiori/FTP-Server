...
static const char *answers[][2] = {
    ...
    },
    {
        ...
        "or retrieval without altering his login or "
    ...
    {
        "NOOP", "214 - This command does not affect any parameters "
        "or previously entered commands. It specifies no action other "
        "than that the server send an OK reply.\r\n"
    },
    {
    ...
    },
    {
        ...
        "on a data port (which is not its default data port) and to wait for "
        "a connection rather than initiate one upon receipt of a "
    ...
    },
    {
        ...
        "copy of the file, specified in the pathname, to the server- "
        "or user-DTP at the other end of the data connection.\r\n"
    },
    {
    ...
    },
    {
        ...
        "identifying the user. The user identification is that which is "
        ...
        "after the control connections are made (some servers may require "
        "this).\r\n"
    },
    {
        NULL, NULL
...
void help_cmd(client_t *client, const char *buffer)
{
    char cmd[MAX_SIZE];
    char *list[] = {CMD_NAMES, NULL};
    int cmd_len;

    cmd_len = sscanf(buffer, "HELP %s", cmd);
    for (size_t i = 0; answers[i][0] != NULL; i++) {
        if (strcmp(answers[i][0], cmd) == 0) {
            client->response = (char *)answers[i][1];
            return;
        }
    }
    client->response = "214 Help\r\n";
    return;
}
